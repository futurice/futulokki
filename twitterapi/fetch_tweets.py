...
def main():
    ...
    tweets = '[]'
    while tweets.strip() == '[]':
        tweets = api.user_tweets('FutuLokki')

        if tweets == '[]':
            time.sleep(5)

    write_tweets_to_file('at_futulokki.json', tweets)

    tweets = '[]'
    while tweets.strip() == '[]':
        tweets = api.search_tweets('#FutuLokki')

        if tweets == '[]':
            time.sleep(5)

    write_tweets_to_file('hashtag_futulokki.json', tweets)
